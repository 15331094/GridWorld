...
public class Jigsaw {
	...
	public boolean BFSearch() throws IOException {
		...
		this.closeList.add(this.beginJNode);
		searchedNodesNum++;
		while (openList.size() != 0 && isCompleted != true) {
			currentJNode = openList.remove(0);
			int[] dir = currentJNode.canMove();
	...
	public boolean isInClose(JigsawNode cur) {
		if (closeList.size() == 0) {
			return false;
		}
		boolean flag = false;
		for (JigsawNode temp: closeList) {
			if (temp.equals(cur)) {
				flag = true;
			}
		}
	...
	private void estimateValue(JigsawNode jNode) {
        ...
        for(int i = 1; i <= dimension*dimension; i++){  
            for(int j = 1; j <= dimension*dimension; j++ ){  
                if(jNode.getNodesState()[i] != 0 && jNode.getNodesState()[i] == endJNode.getNodesState()[j]){  
                    wrongPosition++;
                    int x = getPositive((i-1)/5 - (j-1)/5);
                    int y = getPositive((i+4)%5 - (j+4)%5);
                    cirDis += ( x + y );  
        ...
        int total = 3*cirDis+s+2*lineDis;
